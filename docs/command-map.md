...
| Login intermittently returns `419 Page Expired` | `make status-strict PLAYBOOK_FLAGS=--ask-become-pass` | APP_KEY/session-config drift, Redis session durability, or cross-node session continuity | Require the shared-session probes to pass; then reload `/login` once to replace the already-expired browser token |
