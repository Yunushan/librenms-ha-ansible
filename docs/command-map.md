...
| Reassert an existing maintenance hold | `playbooks/maintenance-hold.yml` | `make maintenance-hold MAINTENANCE_TARGET=lnms1` | A reachable node already in `maintenance_nodes` started a worker or HA repair unit | Removing the node from maintenance or manually starting data services |
...
| Session checks appeared after a code pull during one-node maintenance | `playbooks/session-continuity-repair.yml` | `make session-repair-ask-become-pass SESSION_REPAIR_CONFIRM=true` | Active Redis nodes need live AOF adoption or active web nodes need anchored APP_KEY/session-cache reconciliation without full Galera convergence | Removing the maintained node from inventory, rotating APP_KEY, or restarting Redis manually |
...
| Optional AI assistant deployment | `playbooks/ai-assistant.yml` | `make ai-assistant-ask-become-pass` | An allowlisted local or hosted model endpoint and vaulted credential are ready | Putting API keys in browser code or letting a model execute changes |
...
| Login intermittently returns `419 Page Expired` | `make status-strict PLAYBOOK_FLAGS=--ask-become-pass` | APP_KEY/session-config drift, Redis session durability, or cross-node session continuity | During one-node maintenance, run the guarded session repair; require the shared-session probes to pass, then reload `/login` once |
