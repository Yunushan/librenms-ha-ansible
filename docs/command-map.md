...
| Session checks appeared after a code pull during one-node maintenance | `playbooks/session-continuity-repair.yml` | `make session-repair-ask-become-pass SESSION_REPAIR_CONFIRM=true` | Active Redis nodes need live AOF adoption and active web nodes need the Laravel session probe without full Galera convergence | Removing the maintained node from inventory or restarting Redis manually |
