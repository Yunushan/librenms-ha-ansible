...
printf '%s  %s\n' \
  ...
  "$(basename "$EVIDENCE")" > "$EVIDENCE.sha256"
write_hmac_sidecar
if printf '%s' "$APP_KEY" | python3 "$VERIFIER" --evidence "$EVIDENCE" --app-key-stdin >/dev/null 2>&1; then
