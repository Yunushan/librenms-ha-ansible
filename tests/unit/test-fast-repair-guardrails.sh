...
grep -q 'Mask distribution rrdcached socket activation' "${ROOT_DIR}/roles/librenms_app/tasks/main.yml"
grep -q 'masked: true' "${ROOT_DIR}/roles/librenms_app/tasks/main.yml"
grep -q 'Remove the obsolete RRDCacheD socket systemd drop-in' "${RRDCACHED_UNIT_PLAYBOOK}"
...
grep -q 'systemctl mask rrdcached.socket' "${FAST_REPAIR_DOC}"
