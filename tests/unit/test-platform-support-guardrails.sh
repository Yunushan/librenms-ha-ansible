...
require_text "$DEFAULTS_FILE" 'librenms_managed_python_system_binary: ""'
require_text "$DEFAULTS_FILE" 'librenms_python_315_preview_enabled: false'
require_text "$DEFAULTS_FILE" 'librenms_python_315_min_ansible_core_version: "2.22.0"'
...
require_text "$DEFAULTS_FILE" 'librenms_ubuntu_php_version_override: ""'
require_text "$DEFAULTS_FILE" '  - "8.4"'
require_text "$DEFAULTS_FILE" '  - "8.5"'
require_text "$REDHAT_REPOS_FILE" 'PHP 8.4 and 8.5 require the explicitly enabled'
...
require_text "$PACKAGE_SMOKE_FILE" 'LIBRENMS_PHP_STREAM'
require_text "$PACKAGE_SMOKE_FILE" 'php${php_expected_stream}-'
require_text "$WORKFLOW_FILE" 'ubuntu-24.04-php-8.4'
require_text "$WORKFLOW_FILE" 'ubuntu-24.04-php-8.5'
...
require_text "$WORKFLOW_FILE" 'id: python-315-controller'
...
require_text "$WORKFLOW_FILE" 'requirements-ci.in'
require_text "$WORKFLOW_FILE" 'python-version: "${{ steps.controller-python.outputs.version }}"'
require_text "$BOOTSTRAP_FILE" 'Guard Python 3.15 target support before switching interpreter'
require_text "$BOOTSTRAP_FILE" 'librenms_platform_python_315_preview_enabled'
...
require_text "$WORKFLOW_FILE" 'id: python-315'
...
require_text "$MANAGED_RUNTIME_SCRIPT" 'case "${PYTHON_315_PREVIEW_ENABLED}" in'
require_text "$MANAGED_RUNTIME_SCRIPT" 'librenms_python_315_preview_enabled=true'
require_text "$ROOT_DIR/Makefile" 'PYTHON_BIN ?= python3'
require_text "$ROOT_DIR/Makefile" 'PYTHON_BIN="$(PYTHON_BIN)" bash scripts/bootstrap-controller.sh'
require_text "$ROOT_DIR/Makefile" '$(PYTHON_BIN) scripts/ci-python-smoke.py'
require_text "$ROOT_DIR/Dockerfile" 'ARG CONTROLLER_PYTHON_BASE_IMAGE='
require_text "$ROOT_DIR/Dockerfile" 'FROM ${CONTROLLER_PYTHON_BASE_IMAGE}'
...
require_text "$REDIS_SENTINEL_TASKS_FILE" 'Inspect native Sentinel service after legacy custom-service stop'
require_text "$REDIS_SENTINEL_TASKS_FILE" 'librenms_redis_sentinel_native_state_after_custom_stop.status.ActiveState'
require_text "$REDIS_SENTINEL_TASKS_FILE" 'Inspect project-managed Sentinel service after native-service stop'
require_text "$REDIS_SENTINEL_TASKS_FILE" 'librenms_redis_sentinel_custom_state_after_native_stop.status.ActiveState'
